...
def inject():
    ...
    # Search for "const vocabularyData = {"
    if 'bundle4:' in html:
        print("Bundle 4 already exists in vocabulary-cards.html")
        return

    idx = html.find('const vocabularyData = {')
    if idx == -1:
        print("Could not find vocabularyData object")
        return
    
    # Insert right after the opening brace
    insert_idx = idx + len('const vocabularyData = {') + 1
    new_html = html[:insert_idx] + get_bundle4_vocab() + html[insert_idx:]

    with open('vocabulary-cards.html', 'w', encoding='utf-8') as f:
        f.write(new_html)

    print("Successfully injected Bundle 4 vocabulary.")
