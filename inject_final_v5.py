...
def inject():
    ...
    # Escape quotes by switching to backticks
    combined_js = re.sub(r"content:\s*'(.*?)'", r"content: `\1`", combined_js, flags=re.DOTALL)
    combined_js = re.sub(r"title:\s*'(.*?)'", r"title: `\1`", combined_js, flags=re.DOTALL)
    combined_js = re.sub(r"subtitle:\s*'(.*?)'", r"subtitle: `\1`", combined_js, flags=re.DOTALL)
    combined_js = re.sub(r"term:\s*'(.*?)'", r"term: `\1`", combined_js, flags=re.DOTALL)
    combined_js = re.sub(r"definition:\s*'(.*?)'", r"definition: `\1`", combined_js, flags=re.DOTALL)
